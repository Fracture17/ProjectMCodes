...
void gameplayFixes(Fighter* fighter, int pNum) {
    ...
    && statMod->action == 0xB
    && prevAction == 0xA) {
        posModule->yPos += kinModule->getSumSpeed().yPos;
        _updatePosture_StageObject(fighter, true);
        _updateRoughPos_StageObject(fighter);
        _updateNodeSRT_StageObject(fighter);
    ...
    && anmData.animFrame == 0
    && statMod->action == 0x3) {
        posModule->xPos += kinModule->getSumSpeed().xPos;
        _updatePosture_StageObject(fighter, true);
        _updateRoughPos_StageObject(fighter);
        _updateNodeSRT_StageObject(fighter);
    }
    ...
    && fighter->getOwner()->aiInputPtr->ftEntryPtr->charId == CHAR_ID::Yoshi
    && (anmData.animFrame >= 0 && anmData.animFrame <= 3)
    && statMod->action == 0x33
    && prevAction == 0xC) {
        // kinModule
        auto& energies = kinModule->kineticEnergyVector->energies;
        soKineticEnergyNormal& gravityEnergies = **(energies.at(1));
        if (anmData.animFrame == 0) {
            if (gravityEnergies.yVel >= 1.8) gravityEnergies.yAccel = ftParams->gravity * -3;
            else if (gravityEnergies.yVel >= 0.7) gravityEnergies.yAccel = ftParams->gravity * -5;
        }
        if (gravityEnergies.yAccel < ftParams->gravity) gravityEnergies.yAccel += ftParams->gravity;
        else gravityEnergies.yAccel = ftParams->gravity * -1;
    }
...
extern "C" void updateUnpaused(soModuleAccessor* accessor) {
    ...
    //     }
    // }
    if (currData.debug.enabled || SCENE_NAME == 0x71547261) {
